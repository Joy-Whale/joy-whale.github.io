...
- 删除无用的Drawable文件
随着版本的不断迭代，项目中会产生很多已经弃用的Drawable文件，但由于众多原因（例如开发人员替换、担心项目出现问题）导致这些无用的Drawable文件一直保存在我们项目中，但这是瘦身中不得不做的一件事情。使用Android Studio自带的功能可以帮我们解决这个难题，点击**Analyze > Run Inspection by Name > Unused Resources**，Android Studio会找出那些代码中或XML文件中不再使用的Drawable文件，但我们还需要删除一些无用的调用Drawable文件的代码，否则的话，这些代码中调用的Drawable是不会被列出来的。

- 图片压缩
Drawable文件中占大头的往往是那些PNG、JPG图片，例如开屏图或是引导图，往往一张图就100k以上，而这些图片通常能压缩到几十k甚至十几k左右。在日常开发中，设计发过来的图片我都会进行压缩后再去使用。不用担心压缩会带来图片质量上的问题，现在很多好的压缩工具在不损失图片本身质量的同时会压缩图片20%以上的大小。图片压缩工具很多，例如在线网站[TinyPng](https://tinypng.com/)、Google出品的压缩工具[zopfli](https://github.com/google/zopfli)等。
...
- 使用JPG代替PNG
用JPG代替PNG，由于JPG没有Alpha通道，所以文件更小，适用于不需要透明度的图片可以考虑，通常一个几百k甚至1M的PNG图片转化为JPG文件并做压缩，结果仅仅只有十几到几十K左右，并且显示效果并没有什么差别。

- 使用WebP代替PNG
在Android Studio 2.3中添加了对WebP的支持。一个无损的WebP图片比一个PNG图片体积小约25%，在Android Studio中只需要右击一个PNG图片或WebP图片即可来回切换它们的格式，使用起来是非常方便的。
